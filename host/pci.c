...
enum nvme_legacy_perf_dir {
	NVME_LEGACY_PERF_READ = 0,
	NVME_LEGACY_PERF_WRITE,
	NVME_LEGACY_PERF_NR,
};
...
struct nvme_wb_ctx {
	...
	atomic64_t perf_work_calls;
	atomic64_t perf_work_ns;
	atomic64_t perf_work_segs;
	atomic64_t perf_work_bytes;
	atomic64_t perf_queue_wait_ns;
	atomic64_t perf_mcp_read_ns;
	...
	u64 perf_last_work_calls;
	u64 perf_last_work_ns;
	u64 perf_last_work_segs;
	u64 perf_last_work_bytes;
	u64 perf_last_queue_wait_ns;
	u64 perf_last_mcp_read_ns;
	...
	u64 perf_last_mcp_ready_cnt;
	u64 perf_last_mcp_parse_fail_cnt;
	u64 perf_last_copy_done_submit_cnt;
	u64 perf_last_read_done_submit_cnt;
	u64 perf_last_fallback_cnt;
};

struct nvme_legacy_perf_ctx {
	atomic64_t ios[NVME_LEGACY_PERF_NR];
	atomic64_t bytes[NVME_LEGACY_PERF_NR];
	atomic64_t prep_ns[NVME_LEGACY_PERF_NR];
	...
	spinlock_t perf_lock;
	u64 perf_last_log_ns;
	u64 perf_last_ios[NVME_LEGACY_PERF_NR];
	u64 perf_last_bytes[NVME_LEGACY_PERF_NR];
	u64 perf_last_prep_ns[NVME_LEGACY_PERF_NR];
...
#if NVME_WB_HOST_PERF_ENABLE
static void nvme_legacy_perf_ns_to_us2(u64 ns, u64 *us_int, u64 *us_frac)
{
	*us_int = div64_u64(ns, 1000);
	*us_frac = div64_u64(ns % 1000, 10);
}

static void nvme_legacy_io_perf_log_if_due(struct nvme_dev *dev)
{
	struct nvme_legacy_perf_ctx *legacy = &dev->legacy;
	u64 now = ktime_get_ns();
	u64 cur_ios[NVME_LEGACY_PERF_NR];
	u64 cur_bytes[NVME_LEGACY_PERF_NR];
	u64 cur_prep_ns[NVME_LEGACY_PERF_NR];
	...
	u64 submit_us_frac[NVME_LEGACY_PERF_NR] = { 0 };
	u64 device_us_int[NVME_LEGACY_PERF_NR] = { 0 };
	...
	if (likely(now - READ_ONCE(legacy->perf_last_log_ns) < NSEC_PER_SEC))
		return;

	spin_lock(&legacy->perf_lock);
	if (now - legacy->perf_last_log_ns < NSEC_PER_SEC) {
		spin_unlock(&legacy->perf_lock);
		return;
	}

	for (dir = 0; dir < NVME_LEGACY_PERF_NR; dir++) {
		cur_ios[dir] = atomic64_read(&legacy->ios[dir]);
		cur_bytes[dir] = atomic64_read(&legacy->bytes[dir]);
		cur_prep_ns[dir] = atomic64_read(&legacy->prep_ns[dir]);
		...
		d_prep_ns[dir] = cur_prep_ns[dir] - legacy->perf_last_prep_ns[dir];
		...
		if (d_ios[dir]) {
			...
			nvme_legacy_perf_ns_to_us2(avg_prep_ns[dir],
						&prep_us_int[dir],
						&prep_us_frac[dir]);
			nvme_legacy_perf_ns_to_us2(avg_submit_ns[dir],
						&submit_us_int[dir],
						&submit_us_frac[dir]);
			nvme_legacy_perf_ns_to_us2(avg_device_ns[dir],
						&device_us_int[dir],
						&device_us_frac[dir]);
	...
	total_ios = d_ios[NVME_LEGACY_PERF_READ] + d_ios[NVME_LEGACY_PERF_WRITE];
	legacy->perf_last_log_ns = now;
	for (dir = 0; dir < NVME_LEGACY_PERF_NR; dir++) {
		legacy->perf_last_ios[dir] = cur_ios[dir];
		legacy->perf_last_bytes[dir] = cur_bytes[dir];
		legacy->perf_last_prep_ns[dir] = cur_prep_ns[dir];
	...
	dev_info(dev->ctrl.device,
		...
		submit_us_frac[NVME_LEGACY_PERF_READ],
		device_us_int[NVME_LEGACY_PERF_READ],
		...
		submit_us_frac[NVME_LEGACY_PERF_WRITE],
		device_us_int[NVME_LEGACY_PERF_WRITE],
		...
		struct request *req, u64 complete_start_ns, u64 complete_end_ns)
{
	...
	iod->legacy_perf_tracked = false;
	if (!iod->legacy_issue_ns || !iod->legacy_prep_end_ns ||
	    !iod->legacy_submit_ns)
		return;

	dir = iod->legacy_perf_is_write ? NVME_LEGACY_PERF_WRITE :
		NVME_LEGACY_PERF_READ;
	prep_ns = iod->legacy_prep_end_ns - iod->legacy_issue_ns;
...
static void nvme_wb_perf_log_if_due(struct nvme_dev *dev)
{
	...
	u64 now = ktime_get_ns();
	u64 d_work_calls, d_work_ns, d_work_segs, d_work_bytes;
	u64 d_queue_wait_ns;
	...
	u64 cur_complete_ns;
	u64 cur_mcp_ready, cur_mcp_parse_fail, cur_copy_done_submit;
	u64 cur_read_done_submit, cur_fallback;
	u64 notify_backlog = 0;
	u64 notify_backlog_peak = 0;

	if (likely(now - READ_ONCE(wb->perf_last_log_ns) < NSEC_PER_SEC))
		return;

	spin_lock(&wb->perf_lock);
	if (now - wb->perf_last_log_ns < NSEC_PER_SEC) {
		spin_unlock(&wb->perf_lock);
		return;
	}

	cur_work_calls = atomic64_read(&wb->perf_work_calls);
	cur_work_ns = atomic64_read(&wb->perf_work_ns);
	cur_work_segs = atomic64_read(&wb->perf_work_segs);
	...
	cur_copy_ns = atomic64_read(&wb->perf_copy_ns);
	cur_notify_ns = atomic64_read(&wb->perf_notify_ns);
	cur_complete_ns = atomic64_read(&wb->perf_complete_ns);

	cur_mcp_ready = atomic64_read(&wb->mcp_ready_cnt);
	cur_mcp_parse_fail = atomic64_read(&wb->mcp_parse_fail_cnt);
	cur_copy_done_submit = atomic64_read(&wb->copy_done_submit_cnt);
	cur_read_done_submit = atomic64_read(&wb->read_done_submit_cnt);
	cur_fallback = atomic64_read(&wb->fallback_cnt);

	if (wb->qctx) {
		u32 qid;

		for (qid = 1; qid <= wb->nr_io_queues_layout; qid++) {
			notify_backlog += READ_ONCE(wb->qctx[qid].notify_pending_cnt);
			notify_backlog_peak = max_t(u64, notify_backlog_peak,
				READ_ONCE(wb->qctx[qid].notify_pending_hwm));
		}
	}

	d_work_calls = cur_work_calls - wb->perf_last_work_calls;
	d_work_ns = cur_work_ns - wb->perf_last_work_ns;
	d_work_segs = cur_work_segs - wb->perf_last_work_segs;
	...
	d_copy_ns = cur_copy_ns - wb->perf_last_copy_ns;
	d_notify_ns = cur_notify_ns - wb->perf_last_notify_ns;
	d_complete_ns = cur_complete_ns - wb->perf_last_complete_ns;

	d_mcp_ready = cur_mcp_ready - wb->perf_last_mcp_ready_cnt;
	d_mcp_parse_fail = cur_mcp_parse_fail - wb->perf_last_mcp_parse_fail_cnt;
	d_copy_done_submit = cur_copy_done_submit - wb->perf_last_copy_done_submit_cnt;
	...
	if (d_work_calls) {
		...
		queue_wait_ns = div64_u64(d_queue_wait_ns, d_work_calls);
		...
		queue_wait_us_int = div64_u64(queue_wait_ns, 1000);
		queue_wait_us_frac = div64_u64(queue_wait_ns % 1000, 10);
	...
	if (d_notify_calls) {
		notify_qwait_ns = div64_u64(d_notify_qwait_ns, d_notify_calls);
		notify_ns = div64_u64(d_notify_ns, d_notify_calls);

		notify_qwait_us_int = div64_u64(notify_qwait_ns, 1000);
		notify_qwait_us_frac = div64_u64(notify_qwait_ns % 1000, 10);
	...
	dev_info(dev->ctrl.device,
		...
		queue_wait_us_int, queue_wait_us_frac,
	...
	wb->perf_last_work_ns = cur_work_ns;
	wb->perf_last_work_segs = cur_work_segs;
	...
	wb->perf_last_copy_ns = cur_copy_ns;
	wb->perf_last_notify_ns = cur_notify_ns;
	wb->perf_last_complete_ns = cur_complete_ns;
	wb->perf_last_mcp_ready_cnt = cur_mcp_ready;
	wb->perf_last_mcp_parse_fail_cnt = cur_mcp_parse_fail;
	wb->perf_last_copy_done_submit_cnt = cur_copy_done_submit;
	wb->perf_last_read_done_submit_cnt = cur_read_done_submit;
	wb->perf_last_fallback_cnt = cur_fallback;
...
static inline void nvme_legacy_io_perf_log_if_due(struct nvme_dev *dev)
{
	(void)dev;
}

static inline void nvme_legacy_io_perf_complete(struct nvme_dev *dev,
		struct request *req, u64 complete_start_ns, u64 complete_end_ns)
{
	(void)dev;
	(void)req;
	(void)complete_start_ns;
	(void)complete_end_ns;
}
#endif
		...
		struct request_queue *q, u16 qid, u16 cmd_id, u32 seg_cnt)
{
	...
	ret = __nvme_submit_sync_cmd(q, &cmd, NULL, NULL, 0,
			0, qid, 0,
...
static bool nvme_wb_queue_async_notify(struct nvme_dev *dev,
		struct request_queue *q, u16 qid, u16 cmd_id,
		u32 seg_cnt, bool is_write)
{
	...
	if (wb_notify_hwm_warn && qctx->notify_pending_cnt >= wb_notify_hwm_warn) {
		now = ktime_get_ns();
		if (now - qctx->notify_last_warn_ns >= NSEC_PER_SEC) {
			qctx->notify_last_warn_ns = now;
			dev_warn(dev->ctrl.device,
				"[WB-HOST] notify backlog high-water qid=%u pending=%u threshold=%u\n",
...
static void nvme_wb_workfn(struct work_struct *work)
{
	...
	struct nvme_queue *nvmeq = req->mq_hctx->driver_data;
	struct nvme_dev *dev = nvmeq->dev;
	struct request_queue *rq_q = req->q;
	struct nvme_wb_queue_ctx *qctx;
	struct nvme_wb_mcp_entry e;
	u32 seg_cnt = 0;
	bool done = false;
	u64 t0 = ktime_get_ns();
	u64 local_queue_wait_ns = 0;
	u64 local_mcp_read_ns = 0;
	u64 local_parse_ns = 0;
	u64 local_copy_ns = 0;
	u64 local_notify_ns = 0;
	u64 local_complete_ns = 0;
	u64 local_bytes = 0;
	u64 t_part;

	if (item->enqueue_ns && t0 > item->enqueue_ns)
		local_queue_wait_ns = t0 - item->enqueue_ns;
	...
	while (!done) {
		...
		if (!nvme_wb_read_mcp_entry(dev, qctx, qctx->mcp_head, &e)) {
			local_mcp_read_ns += ktime_get_ns() - t_part;
			dev_warn(dev->ctrl.device,
		...
		if (unlikely(cmd_id != item->cmd_id || qid != item->qid ||
			     ...
			     !len || !hmb_vaddr)) {
			local_parse_ns += ktime_get_ns() - t_parse;
			dev_warn(dev->ctrl.device,
		...
		local_copy_ns += ktime_get_ns() - t_part;
		if (ret) {
			dev_warn(dev->ctrl.device,
...
out_free:
	...
	atomic64_add(local_bytes, &dev->wb.perf_work_bytes);
	atomic64_add(local_queue_wait_ns, &dev->wb.perf_queue_wait_ns);
	atomic64_add(local_mcp_read_ns, &dev->wb.perf_mcp_read_ns);
	atomic64_add(local_parse_ns, &dev->wb.perf_parse_ns);
	atomic64_add(local_copy_ns, &dev->wb.perf_copy_ns);
...
static void nvme_wb_cancel_all_pending(struct nvme_dev *dev)
{
	...
	for (qid = 1; qid <= dev->wb.nr_io_queues_layout; qid++) {
		...
		struct nvme_wb_notify_item *nitem, *ntmp;
		LIST_HEAD(cancel_list);
		LIST_HEAD(notify_cancel_list);
		unsigned long flags;
		...
		if (qctx->notify_wq) {
			spin_lock_irqsave(&qctx->notify_lock, flags);
			list_for_each_entry_safe(nitem, ntmp, &qctx->notify_pending, node) {
		...
		list_for_each_entry_safe(nitem, ntmp, &notify_cancel_list, node) {
			list_del_init(&nitem->node);
			kfree(nitem);
		}
	}
}
...
static int nvme_wb_setup_layout_and_workers(struct nvme_dev *dev)
{
	...
	for (qid = 1; qid <= nr_io_queues; qid++) {
		...
		if (!qctx->notify_wq) {
			snprintf(notify_name, sizeof(notify_name), "nvme-wb-nfy-q%u", qid);
			qctx->notify_wq = alloc_workqueue(notify_name,
...
complete_now:
	{
		u64 complete_start_ns = ktime_get_ns();
		u64 complete_end_ns;

		if (!nvme_try_complete_req(req, cqe->status, cqe->result))
		nvme_pci_complete_rq(req);
		complete_end_ns = ktime_get_ns();
		nvme_legacy_io_perf_complete(dev, req, complete_start_ns,
					 complete_end_ns);
	}
}
		...
		const struct pci_device_id *id)
{
	...
	atomic64_set(&dev->wb.perf_work_calls, 0);
	atomic64_set(&dev->wb.perf_work_ns, 0);
	atomic64_set(&dev->wb.perf_work_segs, 0);
	atomic64_set(&dev->wb.perf_work_bytes, 0);
	atomic64_set(&dev->wb.perf_queue_wait_ns, 0);
	atomic64_set(&dev->wb.perf_mcp_read_ns, 0);
	atomic64_set(&dev->wb.perf_parse_ns, 0);
	atomic64_set(&dev->wb.perf_copy_ns, 0);
	atomic64_set(&dev->wb.perf_notify_calls, 0);
	atomic64_set(&dev->wb.perf_notify_queue_wait_ns, 0);
	atomic64_set(&dev->wb.perf_notify_ns, 0);
	atomic64_set(&dev->wb.perf_complete_ns, 0);
	spin_lock_init(&dev->wb.perf_lock);
	dev->wb.perf_last_log_ns = ktime_get_ns();
	for (dir = 0; dir < NVME_LEGACY_PERF_NR; dir++) {
		atomic64_set(&dev->legacy.ios[dir], 0);
		atomic64_set(&dev->legacy.bytes[dir], 0);
		atomic64_set(&dev->legacy.prep_ns[dir], 0);
...
static int nvme_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	dev_info(&pdev->dev, "Hello World from NVMe driver by Guo Yi! Probing device [%04x:%04x]\n",
             id->vendor, id->device);
